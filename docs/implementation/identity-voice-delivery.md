...
状态：2026-09-22已合入release并上线，Android 0.3.0已发布；实际版本、备份及公网验收见[发布验证](identity-voice-release.md)。本页下文保留本地实施和发布前准备的时点记录。用户在现有本机App与三段后端方案后明确要求“落实”。已确认组织者/管理员账号密码登录，参会人不必登录；公共逐字稿只显示说话人编号，登录授权后显示姓名。现有本机保存同意不等于云端保存同意，不自动上传0.2.0样本。
...
- 0.3.1新增[访客直接录音](android-guest-recording.md)：普通录音免登录，选人/声音档案按需登录。有效`record`账号使用受控录音；名单在开始时固定并由服务器校验。无名单的访客或普通成员使用匿名协议，已选人但登录失效时要求重登或显式清空，不静默降级。会中追加、跨组织和扫码未进入本轮。
