...
| POST /session、DELETE /session | 本地认证或既定身份流程；登录/退出 | 本地路线才使用密码；限速、CSRF与撤销见DES-09 |
| GET/POST /users；PATCH /users/{id} | 分页用户；新增/修改；expected_version | user.manage；账号唯一、保护最后管理员 |
...
| POST /meetings/{id}/approvals | 上述版本集合、核对说明 | 自审策略与审核范围；版本变化失效 |
| POST /meetings/{id}/publications | approval_id或有效自动策略、publish_id、动作幂等键 | 排队不代表渠道成功；执行前重查 |
...
会议归属确认、删除/保留锁、单场导出、配置/恢复等接口按其PRD在切片前补齐；不允许用通用PATCH任意改状态绕过专门用例。审批和发布必须固定内容/目标/策略/访问版本；后续编辑不能悄悄改旧发送快照。
