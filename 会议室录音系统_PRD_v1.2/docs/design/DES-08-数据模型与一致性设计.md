...
版本1.2-draft；以下为逻辑模型和建议字段，不是已执行SQL。关系型数据库产品与物理类型待T-01/T-05。正式迁移需另行实现验证。

## 1. 基本约定

ID为稳定不透明标识，接口统一字符串；设备离线会话建议UUID并结合认证device_id唯一约束。时间点用UTC存储、带偏移格式传输，展示用会议时区；持续/偏移统一整数毫秒。缺失日期/人员映射使用null。名字可改，关系不使用姓名作键。

可并发编辑记录带version；重要状态变更保留updated_at和关联事件。正文版本、授权变更和审核不可通过普通覆盖丢失历史。单公司不硬编码业务room或群，但不预建商业多租户计费模型。
...
| Meeting | id,session_id,title,room_snapshot,department_id,owner_user_id,initiator_user_id,claimed_by/at,ownership_status,integrity_status,classification,version,deleted_at | session_id唯一；认领者不回填为真实录音发起人 |
...
| Approval | content_version,target_set_hash,policy_version,access_version,approver_id,status | 相关版本变化后失效；不能只存approved=true |
